			...
			<block v-else>
				<view class="input-group">
					<view class="input-row border">
						...
						<input-box ref="input1" type="text" :verification="['isNull']" :verificationTip="['帐号不能为空']" class="input-box" clearable focus :inputValue="account" v-model="account" placeholder="请输入编号或手机号"></input-box>
					</view>
...
<script>
	...
	export default {
		...
		mounted() {
			...
			this.getSessionId();
			this.getSystemInfo();
			this.isMounted = true; // 避免与onShow重复执行
